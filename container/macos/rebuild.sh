...
CFG="$HOME/.hermes/plugins/hermes-vip/config.yaml"
...
/usr/local/bin/container stop "$CNAME" 2>/dev/null || true
/usr/local/bin/container rm "$CNAME" 2>/dev/null || true
sleep 1
/usr/local/bin/container run -d --name "$CNAME" --memory "$MEM" --cpus "$CPU" $VOLS "$IMAGE" sleep infinity
echo "Done"
